...
const SubmitBtn = ({ onClick, loading, disabled }) => (
  <button onClick={onClick} disabled={loading || disabled}
    className={`w-full py-2.5 rounded-lg text-sm font-semibold transition
      ${!loading && !disabled ? "bg-blue-600 text-white hover:bg-blue-700" : "bg-gray-200 text-gray-400 cursor-not-allowed"}`}>
...
const AddNote = () => {
  ...
  const handleSubmit = async () => {
    if (!subjectId || !form.title) return setMessage({ type: "error", text: "Subject and Title are required." });
    setLoading(true); setMessage(null);
    ...
    const payload = {
      ...
      ...(isDiploma
        ? { modelAnswerUrl: form.modelAnswerUrl, microprojectUrl: form.microprojectUrl }
        : { dkNotesUrl: form.dkNotesUrl, insemUrl: form.insemUrl, endsemUrl: form.endsemUrl }),
    ...
    try {
      ...
      if (res.ok) {
        ...
        setForm({ title: "", fileUrl: "", dkNotesUrl: "", insemUrl: "", endsemUrl: "", modelAnswerUrl: "", microprojectUrl: "" });
        setSubjectId("");
      } else setMessage({ type: "error", text: "Failed. Try again." });
    } catch { setMessage({ type: "error", text: "Server error." }); }
    finally { setLoading(false); }
  ...
  return (
    <FormWrapper title="Add Note">
      <Select label="Course *" value={courseId}
        onChange={e => {
          const s = courses.find(c => String(c.id) === e.target.value);
          setCourseId(e.target.value);
          setCourseName(s ? s.name : "");
        }}
        options={courses} placeholder="Select Course" labelField="name" />
      <Select label="Branch *" value={branchId} onChange={e => setBranchId(e.target.value)} options={branches} placeholder="Select Branch" labelField="name" disabled={!courseId} />
      <Select label="Year *" value={yearId} onChange={e => setYearId(e.target.value)} options={years} placeholder="Select Year" labelField="yearName" disabled={!branchId} />
      ...
      {subjectId && (
        <>
          ...
          <p className="text-xs text-gray-400 uppercase tracking-wide">
            {isDiploma ? "Diploma Fields" : "Degree Fields"}
          </p>

          <Field label="Title *" value={form.title} onChange={e => setForm(p => ({ ...p, title: e.target.value }))} placeholder="e.g. Unit 1 Notes" />
          <Field label="Notes URL" value={form.fileUrl} onChange={e => setForm(p => ({ ...p, fileUrl: e.target.value }))} placeholder="https://drive.google.com/..." />

          {isDiploma ? (
            <>
              <Field label="Model Answer URL" value={form.modelAnswerUrl} onChange={e => setForm(p => ({ ...p, modelAnswerUrl: e.target.value }))} placeholder="https://..." />
              <Field label="Microproject URL" value={form.microprojectUrl} onChange={e => setForm(p => ({ ...p, microprojectUrl: e.target.value }))} placeholder="https://..." />
            </>
          ) : (
            <>
              <Field label="DK Notes URL" value={form.dkNotesUrl} onChange={e => setForm(p => ({ ...p, dkNotesUrl: e.target.value }))} placeholder="https://..." />
              <Field label="InSem Papers URL" value={form.insemUrl} onChange={e => setForm(p => ({ ...p, insemUrl: e.target.value }))} placeholder="https://..." />
              <Field label="EndSem Papers URL" value={form.endsemUrl} onChange={e => setForm(p => ({ ...p, endsemUrl: e.target.value }))} placeholder="https://..." />
            </>
          )}
        </>
      )}

      <SubmitBtn onClick={handleSubmit} loading={loading} disabled={!subjectId || !form.title} />
      <Msg message={message} />
    </FormWrapper>
  );
};
